...
<head>
    <script>
        ...
        $(function () {
             ...
             * *****/
            //点击查询按钮时
            $("#btn_notion_submit").click(function () {
                var notion_name = $.trim($("#input_notion_name").val());
                //封装数据
                dataList.notion_name = encodeURI(notion_name);

                getData($(this), "admin/notion/0/10", dataList);
            });
            ...
            //点击刷新按钮时
            $("#btn_notion_refresh").click(function () {
                //清除数据
                dataList.notion_name = null;
                //获取数据
                //getData($(this), "admin/notion/0/10", null);
            });
            //点击table中的数据时
            $("#table_notion_list").find(">tbody>tr").click(function () {
                trDataStyle($(this));
            });
        });
        //获取分类数据
        function getData(object, url, dataObject) {
            var table = $("#table_notion_list");
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    //显示分类统计数据
                    $("#notion_count_data").text(data.notionCount);
                    if(data.notionList.length > 0) {
                        for (var i = 0; i < data.notionList.length; i++) {
                            var notion_id = data.notionList[i].notion_id;
                            var notion_name = data.notionList[i].notion_name;
                            //显示分类数据
                            tbody.append("<tr><td><input type='checkbox' class='cbx_select' id='cbx_notion_select_" + notion_id + "'><label for='cbx_notion_select_" + notion_id + "'></label></td><td title='" + notion_name + "'>" + notion_name + "</td><td><span class='td_special' title='查看分类详情'><a href='javascript:void(0)' onclick='getChildPage(this)'>详情</a></span></td><td hidden class='notion_id'>" + notion_id + "</td></tr>");
                        }
                        //绑定事件
        ...
        function getChildPage(obj) {
            ...
            if (obj === null) {
                ...
                title = "分类详情";
                url = "notion/" + $(obj).parents("tr").find(".notion_id").text();
            }
        ...
        function getPage(index) {
            getData($(this), "admin/notion/" + index + "/10", dataList);
        }
    </script>
</head>
<body>
<div class="frm_div text_info">
    <div class="frm_group">
        <label class="frm_label" id="lbl_notion_name" for="input_notion_name">公告名称</label>
        <input class="frm_input" id="input_notion_name" type="text" maxlength="50"/>
        <input class="frm_btn" id="btn_notion_submit" type="button" value="查询"/>
        <input class="frm_btn frm_clear" id="btn_clear" type="button" value="重置"/>
        <input class="frm_btn" id="btn_notion_delete" type="button" value="删除"/>
    </div>
    <div class="frm_group_last">
        <input class="frm_btn frm_add" id="btn_notion_add" type="button" value="添加一个公告" onclick="getChildPage(null)"/>
        <input class="frm_btn frm_refresh" id="btn_notion_refresh" type="button" value="刷新公告列表"/>
    </div>
</div>
<div class="data_count_div text_info">
    ...
    <span>公告总数:</span>
    <span class="data_count_value" id="notion_count_data">${requestScope.notionCount}</span>
    <span class="data_count_unit">个</span>
</div>
<div class="table_normal_div">
    <table class="table_normal" id="table_notion_list">
        <thead class="text_info">
        <tr>
            <th><input type="checkbox" class="cbx_select" id="cbx_select_all"><label for="cbx_select_all"></label></th>
            <th>公告名称</th>
            <th>操作</th>
            <th hidden class="notion_id">公告ID</th>
        </tr>
        </thead>
        <tbody>
        <c:forEach items="${requestScope.notionList}" var="notion">
            <tr>
                <td><input type="checkbox" class="cbx_select" id="cbx_notion_select_${notion.notion_id}"><label for="cbx_notion_select_${notion.notion_id}"></label></td>
                <td title="${notion.notion_name}">${notion.notion_name}</td>
                <td><span class="td_special" title="查看公告详情"><a href="javascript:void(0)" onclick="getChildPage(this)">详情</a></span></td>
                <td hidden><span class="notion_id">${notion.notion_id}</span></td>
            </tr>
        </c:forEach>
        </tbody>
    </table>
